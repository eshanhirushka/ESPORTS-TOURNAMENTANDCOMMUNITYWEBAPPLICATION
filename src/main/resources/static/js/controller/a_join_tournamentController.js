...
function loadJoinTournament() {
    ...
    var ajaxConfig = {
        method: "GET",
        url: "http://localhost:8080/api/tournament/findAllStatusAIDJOIN?aid="+session_id+"&t_status=pending",
        async: true
    };
    $.ajax(ajaxConfig).done(function (dataAll, statusText, jxhr) {
        dataAll.forEach(function (data) {
            if (data.t_start_date<=maxDate){
                var html = `<tr id="${data.tid}">
                           ...
                           <td>
                                <button class="btn btn-danger btn-sm btnEnd" disabled>End</button>
                           </td>
                         </tr>`;
                ...
                var html = `<tr id="${data.tid}">
                           ...
                           <td>
                                <button class="btn btn-danger btn-sm btnEnd">End</button>
                           </td>
                         </tr>`;
                $("#tblJoinTournament tbody").append(html);
            }
        ...
        $('.btnUnReg').click(function () {
            ...
            $.ajax({
                type: "POST",
                url: "http://localhost:8080/api/admin_tournament/deleteRegisterTournament?tid="+tid+"&aid="+session_id,
                dataType: "json",
                contentType: "application/json;charset=utf-8",
                success: function (data) {
